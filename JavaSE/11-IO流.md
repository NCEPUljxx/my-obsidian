...
# 2. File 类用于文件和目录的操作，网络传输的文件也用它表示，见 [[13-网络编程]]。
...
![](file:///D:/Java/data/JavaSE/11-IO流/1_io.jpg)
...
![](file:///D:/Java/data/JavaSE/11-IO流/2_copy.jpg)
...
昨天学习了基本的一些流，作为IO流的入门，今天我们要见识一些更强大的流。比如能够高效读写的缓冲流，能够转换编码的转换流，能够持久化存储对象的序列化将对象转为字节流，分布式系统中常用，[[10-集合框架]] 中的对象也可序列化存储。流等等。这些功能更为强大的流，都是在基本的流对象基础之上创建而来的，就像穿上铠甲的武士一样，相当于是对基本流对象的一种增强。
...
计算机要准确的存储和识别各种字符集符号，需要进行字符编码，一套字符集必然至少有一套字符编码。常见字符集有ASCII字符集、GBK字符集、Unicode字符集等。![](file:///D:/Java/data/JavaSE/11-IO流/1_charset.jpg)
...
**转换流是字节与字符间的桥梁！**![](file:///D:/Java/data/JavaSE/11-IO流/2_zhuanhuan.jpg)
...
反之，该字节序列还可以从文件中读取回来，重构对象，对它进行**反序列化**。`对象的数据`、`对象的类型`和`对象中存储的数据`信息，都可以用来在内存中创建对象。看图理解序列化： ![](file:///D:/Java/data/JavaSE/11-IO流/3_xuliehua.jpg)
...
> 📎 **相关笔记**：[[10-集合框架]] · [[12-多线程]] · [[13-网络编程]] · [[JavaWeb/02-数据库与持久层]] · [[JavaWeb/02-数据库与持久层]]
