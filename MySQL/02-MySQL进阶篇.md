...
析查询并创建相应的内部解析树，并对其完成相应的优化如确定表的查询的顺序，是否利用索引的 B+Tree 结构与 [[JavaSE 索引|JavaSE 10-集合框架]] 中 TreeMap 的红黑树类似，都是为高效查找设计。等，
...
示例演示:
A. 查询建表语句 --- 默认存储引擎: InnoDB 引擎的事务实现涉及 [[03-MySQL运维篇]] 中的日志机制。
...
Hash索引
支持范围查询
R-tree(空间索 空间索引是MyISAM引擎的一个特殊索引类型，主要用于地理空间数据类
引） 型，通常使用较少
Full-text(全文 是一种通过建立倒排索引,快速匹配文档的方式。类似于
索引) Lucene,Solr,ES
...
分类 含义 特点 关键字
主键 默认自动创建, 只能
针对于表中主键创建的索引 PRIMARY
索引 有一个
唯一
避免同一个表中某数据列中的值重复 可以有多个 UNIQUE
索引
常规
快速定位特定数据 可以有多个
索引
全文 全文索引查找的是文本中的关键词，而不是比
可以有多个 FULLTEXT
索引 较索引中的值
2.3.2 聚集索引&二级索引
而在在InnoDB存储引擎中，根据索引的存储形式，又可以分为以下两种：
分类 含义 特点
聚集索引(Clustered 将数据存储与索引放到了一块，索引结构的叶子 必须有,而且只
Index) 节点保存了行数据 有一个
二级索引(Secondary 将数据与索引分开存储，索引结构的叶子节点关
可以存在多个
Index) 联的是对应的主键
聚集索引选取规则:
如果存在主键，主键索引就是聚集索引。
...
1 explain select * from tb_user where profession = '软件工程' and age > 30 and status
= '0';
...
1 explain select * from tb_user force index(idx_user_pro) where profession = '软件工
程';
示例演示：
A. use index
1 explain select * from tb_user use index(idx_user_pro) where profession = '软件工
...
程';
C. force index
1 explain select * from tb_user force index(idx_user_pro_age_sta) where profession =
'软件工程';
2.6.6 覆盖索引
...
含义
法
count(主 InnoDB 引擎会遍历整张表，把每一行的 主键id 值都取出来，返回给服务层。
键) 服务层拿到主键后，直接按行进行累加(主键不可能为null)
没有not null 约束 : InnoDB 引擎会遍历整张表把每一行的字段值都取出
count(字 来，返回给服务层，服务层判断是否为null，不为null，计数累加。
段) 有not null 约束：InnoDB 引擎会遍历整张表把每一行的字段值都取出来，返
回给服务层，直接按行进行累加。
count(数 InnoDB 引擎遍历整张表，但不取值。服务层对于返回的每一行，放一个数字"1"
字) 进去，直接按行进行累加。
...
1 SHOW TRIGGERS ;
3). 删除
1 DROP TRIGGER [schema_name.]trigger_name ; -- 如果没有指定 schema_name，默认为当前数
据库 。
4.4.3 案例
...
VALUES
5 (null, 'insert', now(), new.id, concat('插入的数据内容为:
...
对应SQL 锁类型 说明
lock tables xxx read / SHARED_READ_ONLY /
write SHARED_NO_READ_WRITE
与SHARED_READ、
...
SHARED_READ SHARED_WRITE兼容，与
lock in share mode
EXCLUSIVE互斥

```sql
insert 、update、 与SHARED_READ、
delete、select ... for SHARED_WRITE SHARED_WRITE兼容，与
update EXCLUSIVE互斥
...
```sql
SELECT ... LOCK IN SHARE 需要手动在SELECT之后加LOCK IN SHARE
```
...
ReadView生成后才开启。
min_trx_id <= trx_id 如果trx_id不在m_ids中，
成立，说明数据已经提交。
<= max_trx_id 是可以访问该版本的
...
数据库 含义
存储MySQL服务器正常运行所需要的各种信息 （时区、主从、用
mysql
户、权限等）
提供了访问数据库元数据的各种表和视图，包含数据库、表、字段类
information_schema
型及访问权限等
...
8 --start-datatime=date1 --stop-datetime=date2 指定日期间隔内的所有日志。
...
> 📎 **相关笔记**：[[01-MySQL基础篇]] · [[03-MySQL运维篇]] · [[JavaWeb/02-数据库与持久层]]
