...
> MySQL 是主流的关系型数据库，后续 [[JavaWeb/02-数据库与持久层|JavaWeb Mybatis 章节]] 会通过 JDBC/Mybatis 连接 MySQL 进行操作。
...
数据库管 DataBase Management
操纵和管理数据库的大型软件
理系统 System (DBMS)
操作关系型数据库的编程语言，定义了一套操作 Structured Query
SQL
关系型数据库统一标准 Language (SQL)
...
![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210916184347938.png) 
...
<img src="assets/file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210916184532212.png)) 



<img src="assets/file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210916184709776.png)). 在此电脑上，右键选择属性



![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210918161550977.png) 
...
<img src="assets/file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210918161646433.png)). 找到 Path 系统变量, 点击 "编辑"



<img src="assets/file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210918161803544.png)). 选择 "新建" , 将MySQL Server的安装目录下的bin目录添加到环境变量



<img src="assets/file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20210918161954696.png)) 



![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20220209203253220.png) 
...
![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20220209203345746.png) 



![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20220209203431405.png) 
...
![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20220209205157019.png) 
...
![](file:///D:/Java/data/MySQL/01-MySQL基础篇/image-20220209205401563.png) 
...
全称 说明
类
Data Definition 数据定义语言，用来定义数据库对象(数据库，表，
DDL
Language 字段)
Data Manipulation
...
数值
(-2147483648， 大整
INT/INTEGER 4bytes (0，4294967295)
2147483647) 数值
极大
BIGINT 8bytes (-2^63，2^63-1) (0，2^64-1) 整数
值
单精
(-3.402823466 E+38， 0 和 (1.175494351 E- 度浮
FLOAT 4bytes
3.402823466351 E+38) 38，3.402823466 E+38) 点数
值
0 和
(-1.7976931348623157 双精
(2.2250738585072014
E+308， 度浮
DOUBLE 8bytes E-308，
1.7976931348623157 点数
1.7976931348623157
E+308) 值
E+308)
小数
依赖于M(精度)和D(标度) 依赖于M(精度)和D(标度)的 值(精
DECIMAL
的值 值 确定
点数)
1 如:
...
1000-01-01 00:00:00 至 YYYY-MM-DD 混合日期和时
DATETIME 8
9999-12-31 23:59:59 HH:MM:SS 间值
1970-01-01 00:00:01 至 YYYY-MM-DD 混合日期和时
TIMESTAMP 4
2038-01-19 03:14:07 HH:MM:SS 间值，时间戳
1 如:
2 1). 生日字段 birthday
3 birthday date
4
...
1 insert into employee values(2,'2','张无忌','男',18,'123456789012345670','2005-01-
01');
3). 批量添加数据
1 INSERT INTO 表名 (字段名1, 字段名2, ...) VALUES (值1, 值2, ...), (值1, 值2, ...), (值
1, 值2, ...) ;
1 INSERT INTO 表名 VALUES (值1, 值2, ...), (值1, 值2, ...), (值1, 值2, ...) ;
案例：批量插入数据到employee表，具体的SQL如下：
1 insert into employee values(3,'3','韦一笑','男',38,'123456789012345670','2005-01-
...
2 select * from emp where idcard like '_________________X';
...
保证字段值满足某一个条件 CHECK
之后)
用来让两张表的数据之间建立连接，保证数据的一致 FOREIGN
外键约束
性和完整性 KEY
...
16 )comment '员工表';
17
18 INSERT INTO emp (id, name, age, job,salary, entrydate, managerid, dept_id)
VALUES
19 (1, '金庸', 66, '总裁',20000, '2000-01-01', null,5),(2, '张无忌', 20,
'项目经理',12500, '2005-12-05', 1,1),
20 (3, '杨逍', 33, '开发', 8400,'2000-11-03', 2,1),(4, '韦一笑', 48, '开
发',11000, '2002-02-05', 2,1),
21 (5, '常遇春', 43, '开发',10500, '2004-09-07', 3,1),(6, '小昭', 19, '程
...
为有以下几种:
行为 说明
NO 当在父表中删除/更新对应记录时，首先检查该记录是否有对应外键，如果有则不
ACTION 允许删除/更新。 (与 RESTRICT 一致) 默认行为
...
②. 查询指定入职日期之后入职的员工信息
1 select * from emp where entrydate > (select entrydate from emp where name = '方东
白');
5.6.3 列子查询
...
②. 根据部门ID, 查询员工信息
1 select * from emp where dept_id in (select id from dept where name = '销售部' or
name = '市场部');
...
表: emp , dept
连接条件: emp.dept_id = dept.id
1 select e.*, d.name from emp e left join dept d on e.dept_id = d.id where e.age >
40 ;
5). 查询所有员工的工资等级
表: emp , salgrade
连接条件 : emp.salary >= salgrade.losal and emp.salary <= salgrade.hisal
1 -- 方式一
2 select e.* , s.grade , s.losal, s.hisal from emp e , salgrade s where e.salary >=
s.losal and e.salary <= s.hisal;
3 -- 方式二
4 select e.* , s.grade , s.losal, s.hisal from emp e , salgrade s where e.salary
...
表: emp , dept
连接条件 : emp.dept_id = dept.id
1 select avg(e.salary) from emp e, dept d where e.dept_id = d.id and d.name = '研发
部';
8). 查询工资比 "灭绝" 高的员工信息。
①. 查询 "灭绝" 的薪资
...
## 6. 事务的 ACID 特性是 [[02-MySQL进阶篇]] 中 InnoDB 引擎的核心。
...
> 📎 **相关笔记**：[[02-MySQL进阶篇]] · [[MySQL 索引]] · [[JavaWeb/02-数据库与持久层]] · [[JavaWeb/01-Web前端基础]]
