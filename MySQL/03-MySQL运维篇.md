...
作用：①. 灾难时的数据恢复；②. MySQL的主从复制的 binlog 机制是 [[JavaWeb/05-SpringBootWeb案例与登录认证|JavaWeb 项目]] 中读写分离的基础。。在MySQL8版本中，默认二进制日志是开启着
的，涉及到的参数如下：
...
混合了STATEMENT和ROW两种格式，默认采用STATEMENT，在某些特殊情况下会
MIXED
自动切换为ROW进行记录。
...
间。可以通过以下几种方式清理日志：
指令 含义
删除全部 binlog 日志，删除之后，日志编号，将
reset master
从 binlog.000001重新开始
purge master logs to
删除 * 编号之前的所有日志
'binlog.*'
purge master logs before 删除日志为 "yyyy-mm-dd hh24:mi:ss" 之前
'yyyy-mm-dd hh24:mi:ss' 产生的所有日志
...
## 3. 分库分表解决海量数据问题，与 [[JavaSE 索引|JavaSE 12-多线程]] 中的并发控制有相似思路。
...
13 writeType="0" dbType="mysql" dbDriver="jdbc" switchType="1"
slaveThreshold="100">
...
20 writeType="0" dbType="mysql" dbDriver="jdbc" switchType="1"
slaveThreshold="100">
...
29
30 <writeHost host="master" url="jdbc:mysql://192.168.200.214:3306?
useSSL=false&amp;serverTimezone=Asia/Shanghai&amp;characterEncoding=utf8"
user="root" password="1234" />
31 </dataHost>
32 </mycat:schema>
...
属性 取值 含义
设置Mycat的字符集, 字符集需要与MySQL的
charset utf8
字符集保持一致
0为需要密码登陆、1为不需要密码登陆 ,默认
nonePasswordLogin 0,1
为0，设置为1则需要指定默认账户
...
useSqlStat 0,1 -u root -p 查看MyCat执行的SQL, 执行
效率比较低的SQL , SQL的整体执行情况、读
写比例等 ; show @@sql ; show
@@sql.slow ; show @@sql.sum ;
是否开启全局表的一致性检测。1为开启 ，0
useGlobleTableCheck 0,1
为关闭 。
sqlExecuteTimeout 1000 SQL语句执行的超时时间 , 单位为 s ;
用来指定Mycat全局序列类型，0 为本地文
sequnceHandlerType 0,1,2 件，1 为数据库方式，2 为时间戳列方式，默
认使用本地文件方式，文件方式主要用于测试
必须带有MYCATSEQ或者 mycatseq进入序列
sequnceHandlerPattern 正则表达式
匹配流程 注意MYCATSEQ_有空格的情况
子查询中存在关联查询的情况下,检查关联字
subqueryRelationshipCheck true,false
段中是否有分片字段 .默认 false
...
defaultSqlParser 性来指定默认的解析器; 解析器有两个 :
...
processorExecutor MyCat把异步任务交给 businessExecutor
线程池中, 在新版本的MyCat中这个连接池使
用频次不高, 可以适当地把该值调小
指定MySQL协议中的报文头长度, 默认4个字
packetHeaderSize
节
...
txIsolation 1,2,3,4 READ_UNCOMMITED=1;
...
27 <writeHost host="master" url="jdbc:mysql://192.168.200.210:3306?
useSSL=false&amp;serverTimezone=Asia/Shanghai&amp;characterEncoding=utf8"
user="root" password="1234" />
28 </dataHost>
29
30 <dataHost name="dhost2" maxCon="1000" minCon="10" balance="0"
31 writeType="0" dbType="mysql" dbDriver="jdbc" switchType="1"
slaveThreshold="100">
32 <heartbeat>select user()</heartbeat>
33 <writeHost host="master" url="jdbc:mysql://192.168.200.213:3306?
useSSL=false&amp;serverTimezone=Asia/Shanghai&amp;characterEncoding=utf8"
user="root" password="1234" />
34 </dataHost>
35
36 <dataHost name="dhost3" maxCon="1000" minCon="10" balance="0"
37 writeType="0" dbType="mysql" dbDriver="jdbc" switchType="1"
slaveThreshold="100">
38 <heartbeat>select user()</heartbeat>
39 <writeHost host="master" url="jdbc:mysql://192.168.200.214:3306?
useSSL=false&amp;serverTimezone=Asia/Shanghai&amp;characterEncoding=utf8"
user="root" password="1234" />
40 </dataHost>
2). server.xml
...
默认节点 默认节点的所用:枚举分片时,如果碰到不识别的枚举值, 就让它路
defaultNode
...
一个实际的数据库节点被映射为这么多虚拟节点，默认是160倍，也
virtualBucketTimes 就是虚拟节点数是物理节点数的160
倍;virtualBucketTimes*count就是虚拟结点数量 ;
节点的权重，没有指定权重的节点默认是1。以properties文件的
weightMapFile 格式填写，以从0开始到count-1的整数值也就是节点索引为key，
以节点权重值为值。所有权重值必须是正整数，否则以1代替
用于测试时观察各物理节点与虚拟节点的分布情况，如果指定了这个
bucketMapPath 属性，会把虚拟节点的murmur hash值与物理节点的映射按行输出
...
defaultNode 默认节点的所用:枚举分片时,如果碰到不识别的枚举值, 就让它路由到默认节
点 ; 如果没有默认值,碰到不识别的则报错 。
3). 测试
配置完毕后，重新启动MyCat，然后在mycat的命令行中，执行如下SQL创建表、并插入数据，查看数
据分布情况。
...
默认分片(在分片数量定义时, 字符标示的分片编号不在分片数量内时,
defaultPartition
使用默认分片)
...
hashSlice , -1 代表 str.length()-1 , 大于0只代表数字自身 ; 可以理解
...
dateFormat 日期格式
sBeginDate 开始日期
结束日期，如果配置了结束日期，则代码数据到达了这个日期的分片后，会重
sEndDate
复从开始分片插入
...
dateFormat 日期格式
sBeginDate 开始日期
结束日期，如果配置了结束日期，则代码数据到达了这个日期的分片后，会重复
sEndDate
从开始分片插入
3). 测试
配置完毕后，重新启动MyCat，然后在mycat的命令行中，执行如下SQL创建表、并插入数据，查看数
...
全部的readHost 与 备用的writeHost 都参与select 语句的负载均衡（主要针对
1
于双主双从模式）
...
![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20210814180702071.png) 
...
![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20210814181014481.png) 
...
![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20210814182327675.png) 
...
> ​	![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20220105003433870.png)
...
![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20211031230239760.png) 
...
![](file:///D:/Java/data/MySQL/03-MySQL运维篇/image-20211031231930205.png) 
...
> 📎 **相关笔记**：[[02-MySQL进阶篇]] · [[JavaWeb/02-数据库与持久层]]
